...
COMMON_WORD_NONPERSON: set[str] = {
    ...
    # Wave 39 slice-2 (reader-agent audit of 卷233–294, late Tang / 五代十国) — 69 confirmed
    # non-person auto surfaces. The era is dense with 谥号/军号/方镇/封号 fragments and
    # [姓/王/国]+动词 slices the 2-char NER mis-cards. REJECTED as REAL people and kept (NOT
    # banned), each verified corpus-wide: 王景 (乐浪王景 治水 / 楚王景驹), 李固 (东汉太尉),
    # 蔡京 (唐·岭南节度使, not the Song chancellor), 王宗 (御史中丞王宗), 王弘 (刘宋左长史),
    # 王承 (东海太守 / 谯王承), 王霸 (东汉颍川王霸), 杨玄 (仇池氐王杨玄), 周彝 (李周彝),
    # 王尚 (司马尚之, also kept in slice-1), 梁相 (梁国相国 — too ambiguous to globally ban).
    # — 谥号 / 庙谥 片段 —
    "武穆",   # 西平武穆公张轨 / 广平武穆王 — 谥号, the named person follows
    "文献",   # 高密文献王泰 / 始兴文献公王导 — 谥号
    "文昭",   # 文昭皇后 (甄夫人) — 谥号
    "忠献",   # 赐谥忠献 / 吴越忠献王弘佐 — 谥号
    "宣懿",   # 谥韦太后曰宣懿 — 谥号
    "定孝", "惠圣", "惠孝", "闵孝",   # 僖宗惠圣恭定孝 / 懿宗…恭惠孝 / 庄宗…神闵孝 — 帝谥片段
    # — 军号 / 官号 / 官名 glue —
    "昭武",   # 昭武将军 (韩当) — 军号, not 昭(姓)+武
    "龙捷",   # 龙捷都指挥使 / 龙捷兵 — 禁军番号
    "扈驾",   # 扈驾都头 / 扈驾指挥使 — 职掌修饰语
    "惕隐",   # 契丹酋长惕隐 — 契丹官号 (惕隐)
    "骠信",   # 南诏骠信酋龙 — 南诏君主称号
    "唐尹",   # 兴唐尹 (王正言) — 兴唐府尹 (官名片段)
    # — 方镇 / 地名 / 城戍 / 渡口 / 殿名 —
    "成德",   # 成德节度 (方镇) / 贤者恃以成德 (成·德 verb) — never a person
    "徐泗", "徐宿",   # 徐泗观察使 / 忠武徐宿两军 — 方镇·区域名
    "魏府", "秦府",   # 魏府四面都部署 / 儿在秦府 — 府镇·府第
    "梁营",   # 迫梁营挑战 — 梁军营垒
    "延英",   # 延英殿 / 招延英杰 — 殿名 or 招延·英杰
    "杨刘",   # 屯杨刘 / 自杨刘渡河 — 黄河渡口
    "杨村", "杨柳",   # 距杨村五十里 / 屯杨柳桥 — 地名
    "陈桥",   # 宿陈桥 / 屯陈桥 — 地名
    "黄泽",   # 出黄泽 / 扼黄泽路 — 地名/道路
    "黄梅",   # 袭据黄梅 / 破王仙芝于黄梅 — 黄梅县 (place)
    ...
    "王引",   # 汉王引兵 — 汉王 引(led)兵
    "王宜",   # 大王宜率三军 — 大王 宜(should)
    "王亟",   # 王亟杀此九子 / 王亟使救之 — 王 亟(urgently)
    "王会",   # 魏惠王会田 / 会于杜平 / 会于徐州 — 王 会(met)
    "温怒",   # 温怒曰 / 温怒稍解 — 徐温 怒(angry), 徐温 carded in full elsewhere
    "梁诸", "唐诸",   # 与梁诸藩拒 / 唐诸州·诸将 — [国]+诸 mis-slice
    # — 赐姓·姓名 系列 截断 (real people exist only in 3-char full form) —
    "李绍",   # 赐姓名曰李绍荣/绍奇/绍衡 — 后唐赐姓 series, bare 李绍 always a truncation
    "李德",   # 李德裕 / 李德林 / 李德诚 — distinct people wrongly merged under 2-char 李德
    # — 普通词 / 成语 / 年号 / 国号 / 族群 —
    "唐室",   # 公于唐室 / 唐室藩辅·旧臣 — 唐(王)室
    "景福",   # 景福元年 — 年号
    "明德",   # 宣明德化 / 明德侯刘龚 / 明德门 — 「明德」(virtue) or 封号·门名
    "罗平",   # 改元罗平 / 大越罗平国 — 年号·国号 (董昌僭号)
    "周正",   # 用周正(朔) / 大周正乐 — 周历正朔 / 大周·正乐
    "黄钟",   # 黄钟之宫 / 黄钟弦 — 律名
    "钟声",   # 钟声发 — 钟响 (信号声)
    "尔曹",   # 尔曹善教导之 — 第二人称复数 (you all)
    "祖考",   # 思祖考 / 祖考之骨 — 祖先
    "佛骨",   # 迎佛骨 — 佛舍利
    "伏尸",   # 伏尸五十里 — 尸横遍地
    "伏发",   # 伏发，大破贼众 — 伏兵发动
    ...
    "盖欲",   # 其意盖欲… — 语词 盖·欲
    "钱百",   # 钱百五十万缗 — 数额短语
    "赖卿",   # 朕父子赖卿得全 / 赖卿再安 — 依赖·卿 (you)
    "胡语",   # 善胡语 / 能胡语 — 胡人语言
    "吴政",   # 执吴政 / 知吴政 — 吴(国)政事
    "黄旗",   # 举黄旗 — 旗帜
    "黄蛮",   # 黄蛮入寇 — 族群名
    "嗢末",   # 嗢末者,吐蕃之奴号也 — 族群/身份号
    "萨葛",   # 萨葛酋长 / 萨葛都督 — 部族名
    "葛禄",   # 三葛禄,白服突厥 — 部族名 (Qarluq)
    # Wave 39 slice-3 (4 parallel reader agents over the never-systematically-swept
    # middle gap 卷035–232: 西汉后期/三国, 两晋/十六国, 南北朝, 隋唐). 214 confirmed
    # non-person 2-char auto surfaces. Dominant classes: 谥号/庙号/帝后尊号
    # (简文/宣武/道武/武成/文襄/献武/太武/平文/愍怀/则天/神尧…), [姓]+公/侯 honorific
    # merges (曹公/周公/桓公/庄公/董公/袁公/崔公/沈公/陈公/梁公/秦公/赵公/魏公/宋公/
    # 戴侯/吕侯/桓侯…), [爵]+given-name slices (侯渊/侯循/侯勃/侯泰/侯夔/侯韶/侯退/侯正/
    # 侯推/侯骏/侯祗/侯刘/国伦/赵元/朱元/吴修/王义), [实名/国]+动词 (温自/温笑/魏发/魏寇/
    # 魏通/魏加/景至/景欲/景怒/龙飞/龙见/寇齐/唐突/付汝/王自/张弓/祝天/徐观), 可汗/可敦/
    # 部族号 NON-personal (可敦/定杨/胡禄/处月/处密/思结/阿跌/结骨/薛延/乌苏/骑施/叱奴/
    # 纥奚/黜弗), 年号 (更始/景初/景云/景龙/皇泰), places (陈仓/桑乾/徐无/永乐/郁洲/梁邹/
    # 黄虎/彭模/邢浦/谭郊/李闰/朱方/章丘/江右/巩洛/贾胡/苏农/疏勒/阿弩/龙驹/石龙/罗窦/
    # 唐寺/夏首/奚泉/郭外/关右/荆楚/贺真/黄石/祝阿/甘松), and 普通词/成语/器物/制度
    # (诏书/禹贡/孺子/雷同/殷富/余寇/英才/桑梓/吴士/吴寇/胡夷/班剑/单舸/耿耿/尔自/莫二/
    # 朱衣/罗绮/布泉/布绢/汤沐/吉梦/班禄/夏言/魏禅/魏虏/魏末/魏方/魏自/魏南/魏别/梁室/
    # 杜口/瞿然/茅茨/马鞍/马鞭/钩弋/曹郎/杨尹/太弟/上皇/玄元/宣德/奋威/龙腾/秦征/镇之/
    # 沈行/陈诸/陈郑/陈乐/唐有/唐公/梅录/段公/郭公/李公/徐公/刘公/褚公/高子/王诸/呼韩/
    # 尤来/烧何/勒姐/黄泉/章德/恭怀/光烈/和熹/昭烈/魏武/景穆/光文/景昭/文惠/文穆/武元/
    # 显祖/昭成/肃明/宣孝/玄真/节愍/节闵/武昭/秦孝/东昏/任…/周诏/宋元/突董/罗勃).
    # REAL people the agents OVER-FLAGGED, verified corpus-wide & KEPT (NOT banned): the
    # 可汗 personal names 颉利/突利/始毕/处罗/真珠/登里/启民/车鼻/登利/射匮/莫贺/敕连/
    # 达头/木杆/都蓝/佗钵/伏跋/阿波/佗汗/步迦/泥利 (acting khans, alias-heavy), the
    # 封号-of-living-actors 章武(元融)/建宁(李倓)/任城(元澄)/武昭? -> 武昭 IS banned (谥),
    # the given name 义康(刘义康), the surname 仆固(怀恩), the 公主 安乐/玉真/义成/千金/
    # 高阳/郜国 and 武惠(妃), plus plausible bare names 王延/王保; and curated 景侯(韩景侯).
    "更始", "周公", "曹公", "陈仓", "甘松", "关右", "恭怀", "桓公", "昭烈", "魏武",
    "宋公", "戴侯", "刘公", "诏书", "光烈", "和熹", "董公", "奋威", "曹郎", "禹贡",
    "孺子", "王诸", "呼韩", "章德", "庄公", "黄泉", "国伦", "侯刘", "雷同", "祝阿",
    "黄石", "尤来", "烧何", "勒姐", "殷富", "余寇", "永乐", "马鞍", "徐无", "胡夷",
    "英才", "桑乾", "景初", "吕侯", "吴士", "吴寇", "桑梓", "徐公", "袁公", "太弟",
    "简文", "梁邹", "郁洲", "镇之", "景穆", "光文", "宣武", "龙飞", "杨尹", "谭郊",
    "荆楚", "魏公", "太武", "黄虎", "班剑", "秦公", "彭模", "龙腾", "温自", "纥奚",
    "永安", "单舸", "道武", "李闰", "秦征", "魏发", "王义", "高子", "于越", "沈行",
    "耿耿", "桓侯", "温笑", "景昭", "魏营", "魏别", "邢浦", "黜弗", "钩弋", "陈诸",
    "崔公", "尔自", "沈公", "莫二", "东昏", "侯渊", "侯循", "魏南", "节闵", "宣德",
    "显祖", "梁公", "侯勃", "文襄", "寇齐", "文惠", "献武", "魏自", "侯夔", "景至",
    "马鞭", "章丘", "江右", "侯泰", "唐公", "魏加", "定杨", "昭成", "平文", "文穆",
    "景欲", "周诏", "武成", "龙见", "武元", "褚公", "朱衣", "魏方", "侯咨", "朱方",
    "侯韶", "侯退", "贺真", "杜口", "叱奴", "夏首", "班禄", "夏言", "吴修", "魏寇",
    "郭外", "朱元", "奚泉", "吉梦", "汤沐", "魏禅", "布绢", "魏通", "梁室", "景怒",
    "侯正", "付汝", "侯推", "侯骏", "侯祗", "王自", "魏虏", "愍怀", "魏末", "陈公",
    "唐突", "布泉", "石龙", "罗绮", "秦孝", "薛延", "赵元", "上皇", "则天", "皇泰",
    "玄元", "郭公", "李公", "处密", "处月", "思结", "景云", "神尧", "陈郑", "陈乐",
    "景龙", "乌苏", "阿跌", "肃明", "宣孝", "可敦", "胡禄", "唐寺", "马仆", "徐观",
    "赵公", "巩洛", "魏宣", "贾胡", "苏农", "疏勒", "瞿然", "骑施", "玄真", "节愍",
    "阿弩", "武昭", "结骨", "茅茨", "罗窦", "宋元", "龙驹", "突董", "罗勃", "段公",
    "唐有", "张弓", "梅录", "祝天",
}
...
SURNAMES_EXTRA = set(
    ...
    "元"
    # Wave 49 — 姓 absent from the whitelist so their full 姓+名 could not be
    # surname-parsed for the 节-local same-given owner tally (the exact 元-class bug:
    # bare givens dropped/mis-bound, collision guard undercounts owners). All kept OUT
    # of CLEAN_SURNAMES (added to AMBIGUOUS_SURNAMES below) so bare single-char binding
    # still passes the POS gate — each collides with a common 文言 word/title:
