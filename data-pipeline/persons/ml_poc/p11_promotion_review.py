...
def prepare_promotion_review(
    ...
    if output_dir.exists() or output_dir.is_symlink():
        raise FileExistsError(f"promotion review exists: {output_dir}")
    ...
    if (
        ...
        or len(selections) != 37
        or len(selection_keys) != 37
        ...
        raise ValueError("promotion task manifest binding differs")
    ...
    for directory in ("pass-a", "pass-b"):
        ...
        if len(paths) != len(by_name) or set(by_name) != expected_names:
            raise ValueError(f"promotion {directory} inventory differs")
    ...
    with tempfile.TemporaryDirectory(
        ...
        for selection in selections:
            ...
            if task_sha256 != selection.get("task_sha256"):
                raise ValueError(f"promotion task hash differs: {juan}")
            ...
            pack["promotion_evaluation"] = True
            ...
            selected.append({
                "juan": juan,
                "role": "fresh_promotion_evaluation",
        ...
        if totals.get("model_only_review", 0):
            raise ValueError("model candidates entered promotion review")
        review_manifest = {
            "schema_version": 1,
            "status": "round11_fresh_promotion_focused_review",
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Build candidate-blind promotion focused review."
    )
