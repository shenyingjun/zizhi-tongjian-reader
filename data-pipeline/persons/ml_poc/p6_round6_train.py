...
def run_round6_training(dataset_dir: Path, output_dir: Path) -> dict:
    if output_dir.exists() or output_dir.is_symlink():
        raise FileExistsError(f"Round 6 model output exists: {output_dir}")
    ...
    if (
        ...
        raise ValueError("Round 6 dataset inventory differs")
    ...
    if (
        ...
        or manifest.get("status") != "round6_controlled_training_dataset"
        or manifest.get("formal_evaluation") is not False
    ):
        raise ValueError("invalid Round 6 dataset manifest")
    names = {
        "train": "train.jsonl",
        "dev": "dev.jsonl",
    ...
    if any(
        ...
        raise ValueError("Round 6 dataset output hash differs")
    ...
    with tempfile.TemporaryDirectory(
        ...
        if (
            ...
            raise RuntimeError("controlled Round 6 report differs")
        report["round6_control"] = {
        ...
        report["round6_control"]["run_artifacts"] = {
            name: _sha256(staging / name) for name in artifact_names
        }
        report["round6_control"]["input_snapshots"] = {
            name: hashlib.sha256(raw).hexdigest()
            for name, raw in snapshots.items()
        }
        report["claim_limit"] = (
            "Round 6 diagnostic comparison only; Juan 27 and Juan 76 are reused"
        )
        ...
        if (
            ...
            != report["round6_control"]["model_artifact"]
            or any(
                _sha256(staging / name) != digest
                for name, digest
                in report["round6_control"]["run_artifacts"].items()
            )
            or {
                path.name for path in frozen_inputs.iterdir()
            } != expected_names
            or any(
                not path.is_file()
                or path.is_symlink()
                or _sha256(path)
                != report["round6_control"]["input_snapshots"].get(path.name)
                for path in frozen_inputs.iterdir()
            )
        ):
            raise RuntimeError("Round 6 run artifact changed before publication")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run controlled Round 6 model training."
    )
    parser.add_argument("--dataset", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    report = run_round6_training(args.dataset, args.output)
    print(json.dumps({
        "selected_epoch": report["config"]["selected_epoch"],
        "dev": report["dev_challenge"]["exact"],
        "evaluation": report["evaluation"]["exact"],
        "model_artifact": report["round6_control"]["model_artifact"],
