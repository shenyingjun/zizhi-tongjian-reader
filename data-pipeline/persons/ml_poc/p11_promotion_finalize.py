...
EXPECTED_REVIEW_MANIFEST_SHA256 = (
    "cc3965b1ba7c6e13dd40e22b68f64eb6de7f52497e181f22f2ceda07fc559360"
)
EXPECTED_SOURCE_MANIFEST_SHA256 = (
    "ebe06c87674e49eee014624291553cac620b14b42a5ee15ae519c69c16a2d39a"
)
EXPECTED_LOCKED_STATE_ROOT_SHA256 = (
    "3eb62210f94fa1b4323ed414f05a0f400615d172ed9891e1a03de3aa300a353a"
)
EXPECTED_REVIEW_COUNTS = {
    "teacher_disagreement_review": 7,
    "candidate_union": 326,
    "initial_annotations": 319,
    "review_candidates": 21,
    "auto_accepted_consensus": 305,
    "explicit_low_review": 14,
}


def finalize_promotion_reference(
    review_dir: Path,
    state_dir: Path,
    output_dir: Path,
) -> dict:
    if output_dir.exists() or output_dir.is_symlink():
        raise FileExistsError(f"promotion reference exists: {output_dir}")
    ...
    if (
        ...
        != "round11_fresh_promotion_focused_review"
        or manifest.get("promotion_evaluation") is not True
        or manifest.get("formal_evaluation_after_reference_freeze") is not True
        or manifest.get("eligible_for_training") is not False
        or manifest.get(
            "eligible_for_promotion_metric_after_focused_review"
        ) is not True
        ...
        or manifest.get("challenger_locked_before_task_sampling") != "round7"
        ...
        raise ValueError("invalid promotion review manifest")
    ...
    if (
        ...
        raise ValueError("promotion review inventory differs")
    state_snapshots = {
    ...
    if state_root_sha256 != EXPECTED_LOCKED_STATE_ROOT_SHA256:
        raise ValueError("promotion locked state root hash differs")
    ...
    with tempfile.TemporaryDirectory(
        ...
        for selection in selections:
            ...
            final_examples = build_examples(
                ...
                label_provenance=(
                    "candidate_blind_round11_fresh_promotion_reference"
                ),
            )
        ...
        if (
            len(examples) != 37
            or len(identities) != 37
            or sum(decisions_total.values()) != 326
            or sum(focused_decisions.values()) != 21
        ):
            raise ValueError("promotion decision/reference inventory differs")
        examples.sort(key=lambda row: (row["juan"], row["jie_index"]))
        reference_path = staging / "promotion_reference.jsonl"
        _write_jsonl(reference_path, examples)
        report = {
            "schema_version": 1,
            "status": "frozen_round11_fresh_promotion_reference",
            "promotion_evaluation": True,
            "formal_evaluation": True,
            "eligible_for_training": False,
            "eligible_for_promotion_metric": True,
            ...
            "outputs": {
                "promotion_reference_sha256": hashlib.sha256(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Freeze the fresh Round 11 promotion reference."
    )
    parser.add_argument("--review", type=Path, required=True)
    parser.add_argument("--state", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    report = finalize_promotion_reference(
