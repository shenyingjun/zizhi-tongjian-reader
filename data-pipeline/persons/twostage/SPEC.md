...
谱系词与姓名之间常夹一个官名（`其子[牙内诸军使]渥`、`其姪[天雄节度使]继勋`）。此时
只跳过官名、只画其后的 POS·Giv 姓名；被跳过的官名段必须：以官名结尾字（使/尹/令/牧/
守/丞/尉/卿/监/帅/傅/保/师/郎/将/督/射）收尾、不含标点、不含任何人名 token 或 POS·Giv
偏移，且姓名仍须满足右语法边界与既有 封号/复姓/`将军`/`_shi_guard` 守卫。官名段本身
不被 consume，仍可由官名规则单独处理。
...
- `X，Y之子/孙/弟/兄/父……也`
- `官名X，官名Y之亲属词也`；亲属词使用受控谱系词表，两侧只画姓名，官名必须由
  受控官名后缀证明
- `谥曰X`
- `长/次/季/幼/庶曰X`
